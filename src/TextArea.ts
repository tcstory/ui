...
export class TextArea extends Container
{
    ...
    protected init()
    {
        ...
        options.getInput().then((input) =>{
            if(input) {
                input.onFocus.connect(this.startEditing.bind(this));
                input.onInput.connect(this.onInput.bind(this));
                input.onBlur.connect(this.stopEditing.bind(this));
                const {x, y} = this.getGlobalPosition()
                input.onPosition.emit({
                    x,
                    y,
                    width: this.width,
                    height: this.height,
